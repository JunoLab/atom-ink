...
export default class Result {
  ...
  attach () {
    ...
    this.marker = layers[uniqueEditorID(this.editor)].markBufferRange([[this.range[0], 0],
  ...
  static all () {
    ...
    for (let item of atom.workspace.getPaneItems()) {
      ...
      layers[uniqueEditorID(item)].getMarkers().forEach(m => results.push(m.result))
    }
    return results
  }
  ...
  static forLines (ed, start, end, type = 'any') {
    ...
    return layers[uniqueEditorID(ed)].findMarkers({intersectsBufferRowRange: [start, end]})
  ...
  static removeAll (ed = atom.workspace.getActiveTextEditor()) {
    if (!layers[uniqueEditorID(ed)]) return
    layers[uniqueEditorID(ed)].getMarkers().forEach((m) => m.result.remove())
  }
