...
export default class Result {
  constructor (editor, lineRange, opts = {}) {
    ...
    opts.fade = opts.fade != null ? opts.fade : !(Result.removeLines(this.editor, this.range))
  ...
  static removeLines (ed, range, type = 'any') {
    let rs = Result.forLines(ed, range[0], range[1], type)
    if (!rs) return
    rs.map((r) => r.remove())
    rs.length > 0
  }
