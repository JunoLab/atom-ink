...
export default class ResultView {
  ...
  updateWidth (edRect, winWidth) {
    ...
    if (this.shouldRedraw || this.modelUpdated) {
      let w = edRect.width + edRect.left - RESULT_OFFSET - 10 - this.left
      if (w < MIN_RESULT_WIDTH) w = MIN_RESULT_WIDTH
      if (edRect.width > 0 && this.left + RESULT_OFFSET + w > edRect.width + edRect.left) {
        this.getView().style.left = (edRect.width + edRect.left - w - 10 - this.left) + 'px'
