...
export default class ResultView {
  ...
  completeView (opts) {
    ...
    if (!opts.noTB && opts.type === 'inline') {
      let tb = this.toolbar(opts.buttons)
      this.complete.appendChild(tb)
    }
  ...
  setContent (view, {error = false, loading = false}) {
    ...
    if (view.innerHTML.length < 100 && this.model.type === 'inline') {
      this.getView().firstChild && this.getView().firstChild.classList.add('hide')
    } else {
      this.getView().firstChild && this.getView().firstChild.classList.remove('hide')
    }
  }

  toolbar (buttons) {
    let tb = views.render(div('btn-group ink-result-toolbar'))
