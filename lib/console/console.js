...
export default class Console extends PaneItem {
  static activate() {
    ...
    this.subs.add(atom.commands.add('ink-console', {
      ...
      'console:previous-in-history': () => this.getModel().previous(),
      'console:next-in-history': () => this.getModel().next()
    }))
  }
  ...
  focusInput(force) {
    if (this.getInput() != null) { this.emitter.emit('focus-input', force) }
  }
...
//                                                /____/
  ...
  resetPrefix() {
    this.prefix && this.prefix.listener.dispose()
    delete this.prefix
  }
  ...
  moveHistory(up) {
    ...
    setTimeout(() => this.prefix.listener = editor.onDidChange(()=>this.resetPrefix()), 10)
  }
