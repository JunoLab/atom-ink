...
export default class BreakpointManager {
  ...
  insertView(ed, bp) {
    ...
    let marker = ed.markBufferRange([[bp.line-1, 0], [bp.line-1, Infinity]], {invalidate: 'never'})
    // TODO: remove bps if they are past the end of a file
    marker.onDidChange((ev) => {
      let newLine = ev.newHeadBufferPosition.row
    ...
    let view = new BreakpointView({breakpoint: bp})
  ...
  toggleAtSourceLocation ({file, line}) {
    let found = false
    let ind = 0
    for (let bp of this.fileBreakpoints) {
  ...
  toggleActive (bp) {
    if (bp.typ === 'source') {
      let ind = this.fileBreakpoints.indexOf(bp)
      if (ind > -1) {
