...
export default class BreakpointManager {
  constructor(scopes, {toggle, clear, toggleUncaught, toggleException, refresh, addArgs}) {
  ...
  init(ed) {
    ...
    let addClickListener = () => {
      ...
      gutterElement.addEventListener('click', event => {
        ...
        this.tryToggle(ed.getPath(), row + 1)
      })
    }
    addClickListener()
  ...
  insertView(ed, bp) {
    ...
    let view = new BreakpointView({status: bp.isactive ? "active" : "inactive"})
  ...
  setBreakpoints (bps) {
    this.clear()
    bps.forEach(bp => {
      this.add(bp.file, bp.line, bp.isactive)
    })
    this.emitter.emit('update', {breakpoints: bps})
  }

  add (file, line, isactive) {
    let bp = new Breakpoint({file, line, isactive})
  ...
  tryToggle (file, line) {
    ...
    let bp = this.add(file, line, false)
    this._toggle(file, line).then(({response, error}) => {
      this.remove(bp)
      if (error) {
