...
export default class BreakpointManager {
  constructor(scopes, {toggle, clear, toggleUncaught, toggleException, refresh}) {
  ...
  tryToggle (file, line) {
    this._toggle(file, line).then(response => {
      if (response) {
  ...
  tryToggleException () {
    this._toggleException().then(response => {
      if (response) {
        this.onUncaught = onException
      }
    })
  }

  tryToggleUncaughtException () {
    this._toggleUncaught().then(response => {
      if (response) {
