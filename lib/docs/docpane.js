  ...
  itemView (item) {
    ...
    return <div className="item">
             <div className="item-header" style={codeStyle}>
               <span className="typ" title={item.typ}>{this.icon(item)}</span>
               <span className="name" onclick={item.onClickName}>{item.name}</span>
               <span
                 className={`exported icon icon-eye ${item.exported ? "is-exported" : ""}`}
                 title={item.exported ? "exported" : "not exported"}
               />
  ...
  toggleOptions () {
    ...
    if (this.tooltipSubscriptions == null) {
      this.tooltipSubscriptions = new CompositeDisposable(
        atom.tooltips.add(this.refs.toggleLocation.element, {title: 'Search in variable names only'}),
        atom.tooltips.add(this.refs.toggleExported.element, {title: 'Search in exported variables only'}),
        atom.tooltips.add(this.refs.toggleLoaded.element, {title: 'Search in loaded modules only'})
      )
    } else {
  ...
  headerView () {
    return  <div className="header">
      <span className="header-main">
        ...
        <span className="btn-group options-button"><Button ref="options" icon="three-bars" onclick={() => this.toggleOptions()}/></span>
      </span>
      <span ref="headerOptions" className="header-options hidden">
        ...
        <span className="options-btngroup">
          <div className="btn-group">
            <Button ref="toggleLocation" onclick={() => this.toggleLocation()}>names</Button>
            <Button ref="toggleExported" onclick={() => this.toggleExported()}>exported</Button>
