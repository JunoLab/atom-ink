...
function ensurePaneVisible(pane) {
  ...
  return ensurePaneVisible(pane.parent);
};
...
export default class PaneItem {
  ...
  static activate() {
    ...
    return panes.forEach(Pane => Pane.registerView());
  }
  ...
  static registerView() {
    ...
    return subs.add(atom.workspace.addOpener(uri => {
      let m;
  ...
  static fromId(id) {
    ...
    if (pane = this.registered[id]) {
      ...
      pane = this.registered[id] = new this;
      pane.id = id;
      return pane;
    }
  }
  ...
  close() {
    return __guard__(this.currentPane(), x => x.removeItem(this));
  }
};
